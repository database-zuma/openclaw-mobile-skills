---
name: deploy-to-live
description: End-to-end local-to-live deployment workflow. Covers git init, commit, push to GitHub, and deploy to Vercel via CLI. Includes templates for authentication setup. Use when committing code, pushing to GitHub, deploying to Vercel, or any local-to-production pipeline task.
user-invocable: true
...
Generic workflow for shipping any project from local machine to production. Works with any framework (Next.js, Vite, static, etc.). **SETUP REQUIRED**: Configure authentication for your GitHub org and Vercel account.
...
```
Org: YOUR_GITHUB_ORG (replace with your GitHub username or organization)
Scope: repo, workflow
...
# Clone with auth
git clone https://${GITHUB_TOKEN}@github.com/YOUR_GITHUB_ORG/{repo-name}.git

# Set remote with auth (existing repo)
git remote set-url origin https://${GITHUB_TOKEN}@github.com/YOUR_GITHUB_ORG/{repo-name}.git

# Push (bypass credential helper issues)
GIT_CONFIG_NOSYSTEM=1 git -c credential.helper= push \
  "https://x-access-token:${GITHUB_TOKEN}@github.com/YOUR_GITHUB_ORG/{repo-name}.git" main
```

### Vercel CLI Token

```
Token: YOUR_VERCEL_TOKEN (get from https://vercel.com/account/tokens)
Team: YOUR_VERCEL_TEAM (optional - your personal account or team name)
```
...
echo "value" | vercel env add VAR_NAME production --token=YOUR_VERCEL_TOKEN

# List projects
vercel ls --token=YOUR_VERCEL_TOKEN

# Pull project settings
vercel pull --yes --token=YOUR_VERCEL_TOKEN
```
...
git remote add origin https://${GITHUB_TOKEN}@github.com/YOUR_GITHUB_ORG/{repo-name}.git
...
gh repo create YOUR_GITHUB_ORG/{repo-name} --public --source=. --remote=origin --push
...
- **Vercel Token**: Get from https://vercel.com/account/tokens → use in commands as `YOUR_VERCEL_TOKEN`
...
# Load PAT from env
source ~/.openclaw/workspace/.env
git remote add origin https://${GITHUB_TOKEN}@github.com/YOUR_GITHUB_ORG/{REPO_NAME}.git
git push -u origin main
...
- Lives under your personal account or team
...
| Deploy to Vercel | `vercel --prod --yes --token=YOUR_VERCEL_TOKEN` |

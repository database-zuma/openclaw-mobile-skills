...
description: "Chart and graph generation for business reports and data analysis. Creates visual output (PNG/HTML) from sales, performance, and analytical data using matplotlib/plotly. Professional color palette included. Use when user asks for 'chart', 'graph', 'visualization', 'plot', or any visual representation of data beyond text tables."
user-invocable: false
---

# Data Visualization — Business Reports

Skill ini menghasilkan grafik dan chart dari data bisnis atau analisis. Output bisa berupa PNG (untuk laporan) atau HTML interaktif (untuk dashboard).
...
```python
PRIMARY_DARK  = "#2E3440"   # Header, primary dark
PRIMARY_GREEN = "#A3BE8C"   # Highlight, positive, target
PRIMARY_LIGHT = "#ECEFF4"   # Background light
PRIMARY_RED   = "#BF616A"   # Deficit, warning, negative
PRIMARY_GRAY  = "#D8DEE9"   # Secondary, grid lines
PRIMARY_WHITE = "#FFFFFF"

# Palette untuk multi-series
PROFESSIONAL_PALETTE = ["#2E3440", "#A3BE8C", "#81A1C1", "#88C0D0", "#8FBCBB", "#BF616A"]
```
...
def plot_sales_trend(df_monthly, title="Sales Trend", output_path="sales_trend.png"):
    ...
    ax.plot(df_monthly['bulan'], df_monthly['total_sales'],
            color=PRIMARY_DARK, linewidth=2.5, marker='o', markersize=5, label='Actual')

    # Moving average (opsional)
    if len(df_monthly) >= 3:
        df_monthly['ma3'] = df_monthly['total_sales'].rolling(3).mean()
        ax.plot(df_monthly['bulan'], df_monthly['ma3'],
                color=PRIMARY_GREEN, linewidth=1.5, linestyle='--', label='MA 3 bulan')
    ...
    ax.set_title(title, fontsize=14, fontweight='bold', color=PRIMARY_DARK, pad=15)
...
def plot_store_comparison(df, value_col, label_col, title, output_path="bar.png"):
    ...
    # Color bars: top 3 green, bottom 3 red, sisanya gray
    colors = [PRIMARY_GRAY] * len(df)
    colors[-3:] = [PRIMARY_GREEN] * 3   # top 3 (karena ascending)
    colors[:3]  = [PRIMARY_RED] * 3     # bottom 3
    ...
    for bar in bars:
        ...
        ax.text(bar.get_x() + bar.get_width() + val * 0.01,
                ...
                va='center', fontsize=8, color=PRIMARY_DARK)

    ax.set_title(title, fontsize=14, fontweight='bold', color=PRIMARY_DARK)

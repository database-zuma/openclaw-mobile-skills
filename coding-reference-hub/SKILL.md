...
## Multi-LLM Skill Design: Troubleshooting & Patterns

### Lesson: "The Dumber the PROMPT, the Better" (March 2026)

**Context:** RO Request skill worked perfectly on Claude but failed on minimax m2.5. Minimax wrote its own openpyxl code instead of running the pre-built Python script, producing garbage output (2 sheets instead of 5, wrong data, wrong format).

**Root Cause:** PROMPT v4.1 was 264 lines and contained:
- Column names, sheet structures, row counts
- Business rules (protol/box thresholds, surplus tier logic)
- DB credentials (host, port, user, password)
- Validation checklists per sheet
- Troubleshooting section with query examples
- CLI argument reference tables

Weaker LLMs read all this detail and think: *"I have enough info to build this myself"* → write their own code → garbage output.

**Fix:** Strip PROMPT to 53 lines. Keep ONLY:
1. Ask user for inputs (store name, storage)
2. Run script command (one bash block)
3. Verify 5 sheets exist (one python check)
4. Upload to GDrive (one bash block)
5. Share link to user

**Result:** minimax m2.5 immediately started following instructions — ran the script, got correct 5-sheet output.

### Pattern: Script Executor Skill

When a skill produces complex output (Excel, PDF, formatted reports):

```
❌ BAD: "Format Template" skill
   - Describes output format in detail (columns, headers, styling)
   - Explains business logic (rules, thresholds, conditions)
   - Includes DB credentials and query examples
   - Provides troubleshooting with raw SQL
   - Result: Strong LLMs follow it, weak LLMs improvise from it

✅ GOOD: "Script Executor" skill
   - Says ONLY: "run this command with these args"
   - Zero format details (no column names, no sheet names)
   - Zero business logic (no rules, no thresholds)
   - Zero infrastructure details (no DB creds, no SQL)
   - Result: ALL LLMs just run the script — consistent output
```

### Key Principle

```
Info in PROMPT = ammunition for LLM to improvise
Less info = less improvisation = more compliance
```

**The skill PROMPT is NOT documentation.** Documentation goes in separate SKILL.md files inside the script directory. The PROMPT is just a step-by-step execution checklist.

### Checklist: Is Your Skill LLM-Proof?

| Check | Pass | Fail |
|-------|------|------|
| PROMPT under 60 lines? | ✅ | ❌ Too much info |
| Zero column/field names? | ✅ | ❌ LLM will use them to build own output |
| Zero business rules? | ✅ | ❌ LLM will implement rules itself |
| Zero DB/API credentials? | ✅ | ❌ LLM will query directly |
| Script path is absolute? | ✅ | ❌ LLM might not find it |
| "NEVER write your own code" rule? | ✅ | ❌ LLM needs explicit boundary |
| Verification is simple (e.g. count sheets)? | ✅ | ❌ Complex validation = more improvisation |

### When to Apply

Apply this pattern when:
- Skill will be used by **multiple LLMs** (Claude, minimax, Gemini, etc.)
- Output is **structured/formatted** (Excel, PDF, formatted text)
- There's a **working script** that produces correct output
- Consistency matters more than flexibility

Don't apply when:
- Skill is Claude-only (Claude follows complex instructions well)
- Output is freeform text (no fixed format to mess up)
- No pre-built script exists (skill IS the logic)
